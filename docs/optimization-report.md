...
This document summarizes the optimizations made to the Pool Controller
codebase to ensure reliable 24/7 operation and reduce memory leaks.
...
**Solution**: Replaced all dynamic String allocations with stack-based
character buffers.
...
**Issue**: The original code didn't properly handle millis() overflow (occurs
every ~49.7 days).
...
**Problem**: When millis() overflows, the subtraction can produce unexpected
results depending on timing.

**Solution**: Created `Utils::shouldMeasure()` function with proper overflow
handling:

```cpp
inline bool shouldMeasure(unsigned long lastMeasurement,
...
void LoggerNode::logf(const String& function, const E_Loglevel level,
  ...
  va_start(arg, format);
  char temp[100];
  //size_t len = vsnprintf(temp, sizeof(temp), format, arg);  // BUG!
  va_end(arg);
...
void LoggerNode::logf(const String& function, const E_Loglevel level,
  ...
  va_start(arg, format);
  char temp[100];
  vsnprintf(temp, sizeof(temp), format, arg);  // FIXED
  va_end(arg);
...
- `StaticJsonDocument<N>` → `JsonDocument` (uses stack allocation
  automatically)
...
The optimizations made significantly improve the Pool Controller's suitability
for 24/7 operation:
...
These changes ensure the controller can run continuously without memory issues
or timing bugs.
