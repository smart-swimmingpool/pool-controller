...
The planned setup is compatible when all external modules are wired as a 3.3 V
system:

- ESP32-C6 GPIOs are **not 5 V tolerant**; all display and encoder signals must
  stay at 3.3 V logic level.
- ST7789/ILI9341-class TFT controllers are suitable for 3.3 V SPI operation when
  the concrete display module exposes 3.3 V-safe logic pins.
- The KY-040 encoder should be powered from 3.3 V so its CLK/DT/SW pull-ups also
  resolve to 3.3 V.
- Relays and opto inputs remain on the Olimex board's existing circuits; the
  local UI does not alter their power domain.

Power budget from the Olimex hardware analysis:

```text
Olimex supply path:
  8-50 V input -> TX4138 buck -> 5 V rail
  5 V rail     -> SY8089 buck -> 3.3 V rail
...
The 3.3 V rail is generated by a SY8089 buck regulator. The BOM/netlist analysis
indicates the practical limit is set by the 2.2 µH inductor at about 1.5 A
saturation current. For design margin, the local UI should keep the total 3.3 V
load below about 1.2 A.

Estimated added load:

```text
2.4"/2.8" SPI TFT logic + backlight: 60-100 mA typical
KY-040 encoder:                         <1 mA
```

Together with normal ESP32-C6 Wi-Fi peaks, this remains comfortably below the
available 3.3 V rail budget. A separate display power supply is not required for
the proposed TFT + encoder setup.

Required wiring constraints:

- Power display logic, display backlight, and KY-040 from 3.3 V, not 5 V.
- Do **not** drive the TFT backlight directly from an ESP32 GPIO. Use either the
  3.3 V rail directly when the display module includes current limiting, or use a
  GPIO-controlled transistor/MOSFET for PWM dimming.
- Add local decoupling near the display connector, at least 100 nF between 3.3 V
  and GND.
- Verify the concrete TFT module's pinout, backlight polarity, and whether it has
  onboard current limiting or level shifting before connecting it.

No-go conditions:

- A display module that cannot be confirmed safe for 3.3 V logic.
- Any wiring path that can pull ESP32-C6 GPIOs above 3.3 V.
...
For the backlight, “PWM-capable GPIO” means the GPIO only controls a transistor
or MOSFET. The GPIO must not source the backlight current directly.
...
5. Validate relays and opto inputs against the Olimex schematic before connecting
   real pool equipment.
6. Measure or inspect the 3.3 V rail under TFT full-brightness + Wi-Fi load; the
   expected added display/encoder load is about 60-100 mA and should remain well
   below the conservative 1.2 A rail budget.
7. Confirm that no external module feeds 5 V into ESP32-C6 GPIOs.
...
- Olimex ESP32-C6-EVB hardware repository, schematic/BOM/netlist: confirms the
  8-50 V -> 5 V -> 3.3 V regulator chain, UEXT 3.3 V rail connection, and relay
  separation from the 3.3 V rail.
- ESP32-C6 datasheet: confirms 3.3 V GPIO domain and non-5 V-tolerant pins.
- SY8089 regulator documentation and Olimex BOM: supports the 3.3 V rail margin
  estimate, with the inductor saturation current used as the conservative limit.
- TFT controller/module documentation: ST7789/ILI9341-class controllers support
  3.3 V SPI operation, but the exact display module board must still be checked
  for level shifters, onboard regulator, backlight polarity, and current limit.
- KY-040 module documentation: encoder outputs follow the module VCC pull-up,
  therefore use 3.3 V VCC for ESP32-safe levels.
