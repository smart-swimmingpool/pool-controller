...
| 3   | HA finds no device                           | MQTT Discovery payload not published or broker unreachable                      | Verify `mqtt-protocol = "homeassistant"` in settings; check MQTT broker connectivity with MQTT Explorer       |
...
| 5   | Relay behavior inverted                      | Active-low relay module but firmware configured for active-high (or vice versa) | Set `relay-invert = true` or `false` in Configuration → Advanced to match your relay module                   |
...
| 7   | Relay doesn't click                          | GPIO pin damaged, relay module not powered, or optocoupler defective            | Measure GPIO voltage (should toggle); verify relay VCC (5V) and GND; test relay with manual 5V on IN pin      |
| 8   | Safe Mode active                             | Boot-loop detected: 4 consecutive boots < 5 minutes each                        | Read serial log for crash reason (panic, assert, exception); fix the root cause; reboot                       |
...
| 17  | ESP32 not entering AP mode                   | Previous WiFi configuration still stored                                        | Hold BOOT button during power-on (if supported) or perform factory reset via web UI                           |
...
| #   | Symptom            | Likely Cause       | Quick Fix              |
| --- | ------------------ | ------------------ | ---------------------- |
| 3   | Not discovered     | Discovery not sent | Check protocol setting |
...
| #   | Symptom            | Likely Cause      | Quick Fix             |
| --- | ------------------ | ----------------- | --------------------- |
| 5   | Inverted behavior  | Wrong polarity    | Toggle `relay-invert` |
...
- [Build from Zero](/docs/build-from-zero/) — Complete build guide with test procedures
