...
# 🚀 Quick Start: Smart Swimming Pool Controller
...
### 🛒 Shopping List (BOM)
Use the following table to order all required components. **Recommended shops:** Amazon, AliExpress, Reichelt, Pollin, Conrad (DE/AT/CH).
...
| 3 | 2-Channel 5V Relay Module (with optocoupler isolation) | 1 | 5–8€ | **Must be active-high!** (GPIO HIGH → relay ON) | [Amazon DE](https://www.amazon.de/s?k=2+Channel+5V+Relay+Module+active+high), [Reichelt](https://www.reichelt.de/) |
...
| DS18B20 Wire | Connect to | ESP32 Pin |
|:------------:|------------|:---------:|
| VDD (Red)    | 3.3V       | 3V3       |
| GND (Black)  | GND        | GND       |
| DATA (Yellow)| **GPIO32** | GPIO32    | (Solar sensor)
| DATA (Yellow)| **GPIO33** | GPIO33    | (Pool sensor)
...
2. Install the following libraries:
   ...
   - **ESPmDNS** (built-in)
   - **WebServer** (built-in)
   - **LittleFS** (for web interface files)
...
1. Clone the repository:
   ...
   cd pool-controller
   ```
2. Open the **`pool-controller.ino`** file in Arduino IDE.

#### 4️⃣ Configure Board Settings
...
3. Wait for the upload to complete (should see "Done uploading" in the console).
...
#### 2️⃣ Open Project
1. Open the **`pool-controller`** folder in VS Code.
2. PlatformIO will automatically detect the project and install dependencies.

#### 3️⃣ Upload Firmware
1. Connect the ESP32 to your computer via **USB**.
2. Click the **PlatformIO Upload** button (→) in the status bar.
...
3. Turn on the **Pool Pump** and **Solar Pump** switches.
   ...
   - If the relay doesn't click:
     ...
     - Check the **logic level** (active-high vs. active-low).
     - Ensure the **LED on the relay module lights up** when GPIO goes HIGH.
...
| **Relay doesn't click** | Wrong logic level | Use active-high relay or change firmware. |
...
## 📄 License
This guide is part of the **Smart Swimming Pool** project, licensed under the **MIT License**. See [LICENSE](https://github.com/smart-swimmingpool/pool-controller/blob/main/LICENSE) for details.