...
def publish_device_info():
    ...
    mqtt_client.publish(f"{base_topic}/$nodes", "pool-temp,solar-temp,pool-pump,solar-pump,operation-mode", retain=True)
...
def publish_status():
    ...
    try:
        ...
        # Relay states
        mqtt_client.publish(f"{base_topic}/pool-pump/switch", 
                          "true" if pool_pump_node.get_state() else "false")
        mqtt_client.publish(f"{base_topic}/solar-pump/switch", 
