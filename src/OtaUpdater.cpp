...
// The Arduino-ESP32 build system embeds the CA bundle in read-only flash and
// exports it under the linker symbol `_binary_x509_crt_bundle_start`.  The
// extern declaration below gives that symbol the C++ identifier
// `x509_crt_bundle_start` which is then passed to setCACertBundle().
// The pointer must NOT be freed or modified.
