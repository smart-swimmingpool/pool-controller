...
HomieSetting<const char *> LoggerNode::default_loglevel("loglevel", "default loglevel");
HomieSetting<bool> LoggerNode::logserial("logserial", "log to serial");
HomieSetting<bool> LoggerNode::flushlog("flushlog", "Flush serial log after each log");
...
void LoggerNode::onReadyToOperate() {
  PoolController::MqttInterface::publishSelectState(*this, "Level", "log-level", levelstring[m_loglevel].c_str());
  PoolController::MqttInterface::publishSwitchState(*this, "LogSerial", "log-serial", logSerial);
}
...
void LoggerNode::log(const String &function, const E_Loglevel level, const String &text) const {
  ...
  if (Homie.isConnected()) {
    ...
    if (PoolController::MqttInterface::isHomeAssistant()) {
      PoolController::MqttInterface::publishTextState(*this, "log", "log", message.c_str());
    } else {
...
bool LoggerNode::handleHomeAssistantCommand(const char *property, const char *value) {
  this->logf("LoggerNode::handleHomeAssistantCommand()", LoggerNode::DEBUG, "property %s set to %s", property, value);
  const bool retval = applyProperty(String(property), String(value));
  if (!retval) {
    logf("LoggerNode::handleHomeAssistantCommand()", ERROR, "Received invalid property %s with value %s", property, value);
  }
  return retval;
}
