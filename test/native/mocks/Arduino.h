...
public:
  ...
  bool concat(const char *s) { data_ += s; return true; }
  bool concat(const String &s) { data_ += s.data_; return true; }
  ...
  void trim() {
    ...
    if (start == std::string::npos) data_.clear();
    else data_ = data_.substr(start, end - start + 1);
  }
...
public:
  void begin(int) {}
  void print(const char *s) { fprintf(stdout, "%s", s); fflush(stdout); }
  void print(int v) { fprintf(stdout, "%d", v); fflush(stdout); }
  void print(unsigned int v) { fprintf(stdout, "%u", v); fflush(stdout); }
  void print(float v) { fprintf(stdout, "%f", v); fflush(stdout); }
  void print(double v) { fprintf(stdout, "%f", v); fflush(stdout); }
  void println(const char *s) { fprintf(stdout, "%s\n", s); fflush(stdout); }
  void println(int v) { fprintf(stdout, "%d\n", v); fflush(stdout); }
  void println(unsigned int v) { fprintf(stdout, "%u\n", v); fflush(stdout); }
  void println(float v) { fprintf(stdout, "%f\n", v); fflush(stdout); }
  void println(double v) { fprintf(stdout, "%f\n", v); fflush(stdout); }
  void println() { fprintf(stdout, "\n"); fflush(stdout); }
...
public:
  ...
  size_t putUInt(const char *key, unsigned int val) {
    return putInt(key, (int)val);
  }

  unsigned int getUInt(const char *key, unsigned int defaultVal = 0) {
    return (unsigned int)getInt(key, (int)defaultVal);
  }
  ...
  uint8_t getUChar(const char *key, uint8_t defaultVal = 0) {
    return (uint8_t)getInt(key, (int)defaultVal);
  }
  ...
  uint16_t getUShort(const char *key, uint16_t defaultVal = 0) {
    return (uint16_t)getInt(key, (int)defaultVal);
  }
...
public:
  ...
  void _setData(const std::string &d) { data_ = d; valid_ = true; }
...
inline uint32_t constrain(uint32_t x, uint32_t a, uint32_t b) { return x < a ? a : (x > b ? b : x); }
...
public:
  ...
  int read() { return -1; }
  int available() { return 0; }
  size_t readBytes(uint8_t *buf, size_t len) { memset(buf, 0, len); return len; }
  void stop() {}
  void setTimeout(uint32_t) {}
...
inline bool isnan(float x) { return x != x; }  // NaN check
