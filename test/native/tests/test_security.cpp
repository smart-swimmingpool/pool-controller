...
int run_security_tests() {
  ...
  // -- Test: Session token generation produces non-empty string --
  {
    ...
    WebPortal::begin();
    // Generate CSRF token before asserting it (P2 review fix)
  ...
  // -- Test: SSID validation rejects empty SSID --
  {
    ...
    // Test validation logic directly (apiSaveConfig is private)
    // SSID validation: length must be 1-32 bytes
    String emptySsid = "";
  ...
  // -- Test: SSID validation rejects SSID longer than 32 characters --
  {
    ...
    // Build long SSID without fill constructor (not available in mock String)
    String longSsid = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";  // 33 characters
  ...
  // -- Test: SSID validation accepts UTF-8/non-ASCII characters --
  {
    test_begin("Security::InputValidation", "accept SSID with UTF-8 characters");
    ...
    String utf8Ssid = "MyWiFi\xC3\xA9Network";  // Contains UTF-8 character é

    // Check that SSID length is within limits (byte length)
    bool isValidLength = (utf8Ssid.length() > 0 && utf8Ssid.length() <= 32);
    ASSERT_TRUE(isValidLength);

    test_suite_end("Security::InputValidation::utf8-ssid", 1, 0);
    passed++;
  }
  ...
  // -- Test: Password validation rejects password longer than 64 characters --
  {
    ...
    // Build long password without fill constructor (not available in mock String)
    String longPassword = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";  // 65 characters
  ...
  // -- Test: New password validation (length >= 4, aligned with UI) --
  {
    test_begin("Security::Password", "new password length validation");

    ConfigManager::load();

    // Try to set password that's too short (min 4 chars, aligned with UI)
    // This should be rejected by the validation in apiSaveConfig
    // For now, we test the logic directly
    String shortPassword = "abc";
    bool isValidLength = (shortPassword.length() >= 4);
    ASSERT_FALSE(isValidLength);

    // Valid length (4 chars minimum)
    String validPassword = "abcd";
    isValidLength = (validPassword.length() >= 4);
    ASSERT_TRUE(isValidLength);

    // Also test longer passwords
    String longPassword = "long_enough_password";
    isValidLength = (longPassword.length() >= 4);
    ASSERT_TRUE(isValidLength);
